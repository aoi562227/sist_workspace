...
public class JavaMemoEvent extends WindowAdapter implements ActionListener {
    ...
    private void openMemo() {
        ...
        // Load state when opening a memo
        loadState();
    } // openMemo
    ...
    /**
     * Save the state of the memo to a file
     */
    private void saveState() {
        try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(STATE_FILE))) {
        	MemoStateManager memoState = new MemoStateManager(jtaNote.getText(), fontNote);
            oos.writeObject(memoState);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    /**
     * Load the state of the memo from a file
     */
    private void loadState() {
        try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(STATE_FILE))) {
        	MemoStateManager memoState = (MemoStateManager) ois.readObject();
            jtaNote.setText(memoState.getText());
            jtaNote.setFont(memoState.getFont());
